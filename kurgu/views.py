...
import json
from crawler.models import Permision
...
def http_kj_kurgu(request):
    if request.user.is_authenticated:
        try:
            permision=Permision.objects.get(per_type="Kurgu").users.all()
        except:
            Permision(per_type="Kurgu").save()
            permision=Permision.objects.get(per_type="Kurgu").users.all()
        ...
        if request.user in permision:
            ...
            if request.method == "POST":
                ...
                try:
                    ...
                    elif "presentesion_date" in request.POST:
                        object = KJ.objects.get(id=request.POST.get("id"))
                        date=request.POST.get("presentesion_date").split("/")
                        date="%s-%s-%s"%(date[2],date[1],date[0])
...
def http_kj_muhasebe(request):
    if request.user.is_authenticated:
        ...
        try:
            permision=Permision.objects.get(per_type="Muhasebe").users.all()
        except:
            Permision(per_type="Muhasebe").save()
            permision=Permision.objects.get(per_type="Muhasebe").users.all()
        if request.user in permision:
            if request.method == "POST":
